...
from kippy_api import KippyApi, KippyAuthError, KippyResponseError
...
from kippy_api.const import REQUEST_HEADERS, RETURN_VALUES
...
async def test_post_with_refresh_retries_on_expired() -> None:
    ...
    resp1 = _FakeResp(401, f'{{"return": {RETURN_VALUES.AUTHORIZATION_EXPIRED}}}')
...
def test_helper_functions_cover_edge_cases() -> None:
    ...
    assert _tz_hours(datetime.now(UTC)) == 0
    assert _redact_json('{"petID":1}') == '{"petID": "***"}'


@pytest.mark.asyncio
async def test_ensure_login_raises_without_creds() -> None:
...
async def test_post_with_refresh_logs_json(caplog) -> None:
    """Only response metadata is logged."""
    ...
    assert "gps_on_default" not in caplog.text
